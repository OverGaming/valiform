...
function getMessage(messages: RuleOptions['messages']): string {
  ...
  return typeof message === 'function' ? message({}) : (message ?? 'Must contain at least one letter or space')
}
...
export function containsAlphaSpaces(value: unknown, options: Partial<RuleOptions> = {}): true | string {
