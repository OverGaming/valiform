...
function getMessage(messages: RuleOptions['messages']): string {
  ...
  return typeof message === 'function' ? message({}) : (message ?? 'Must contain at least one lowercase letter')
}
...
export function containsLowercase(value: unknown, options: Partial<RuleOptions> = {}): true | string {
