...
<script setup lang="ts">
  ...
  const emit = defineEmits<{
    submit: [values: Record<string, unknown>, helpers: { setErrors: typeof setErrors; reset: typeof reset }]
  }>()
  ...
  const isValid = computed(() =>
    Object.keys(fields.value).every(
      (name) => fields.value[name].isValid as unknown as boolean
    )
  )
  ...
  const setErrors = (...args: unknown[]): void => {
    if (args.length === 1) {
      ...
      errors.value = Array.isArray(errorInput)
        ? (errorInput as string[])
        : [errorInput as string]
